...
def frame_detect(image):
    ...
    # 固定閾値で二値化 黒い部分が255になる
    _, bin_img = cv2.threshold(gaus, 230, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
    # 反転
    bin_img = cv2.bitwise_not(bin_img)
    cv2.imshow('bin_img', bin_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    removed_margin_img, offset = remove_margins(bin_img)
    cv2.imshow('removed_margin_img', removed_margin_img)
    cv2.waitKey(0)
    cv2.destroyAllWindows()
...
if __name__ == '__main__':
    test_img = cv2.imread('./image/comic_dataset/images/ARMS_024.jpg')
